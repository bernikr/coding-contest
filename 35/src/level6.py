...
def parse(tokens: List[str]) -> List[Instruction]:
    ...
    while i < len(tokens):
        match tokens[i]:
            ...
            case 'try':
                blockstack.append(len(output))
                output.append(Instruction(InstType.TRY))
            case 'catch':
                blockstack.append(len(output))
                output.append(Instruction(InstType.CATCH))
            case other:
                assert False, other
        i += 1
    return output
    ...
    for i, ins in enumerate(program):
        if ins.type == InstType.END:
            match program[ins.addr]:
                ...
                case Instruction(type=InstType.TRY):
                    program[ins.addr].addr = i+2
                    ins.link = InstType.TRY
                    ins.addr = i+1
                case Instruction(type=InstType.CATCH):
                    ins.link = InstType.CATCH
                    program[ins.addr].addr = i+1
                    ins.addr = i+1
                case _:
                    pass
        ...
        try:
            match program[pc]:
                ...
                case Instruction(type=InstType.END, link=l, addr=a):
                    if l == InstType.TRY:
                        active_try.pop()
                    if l == InstType.START or l == InstType.IF or l == InstType.ELSE or l == InstType.TRY or l == InstType.CATCH:
                        for nstart, nend in postpone_queues[-1]:
                            res, ret = run_instruction(program, function_map, variables.copy(), start=nstart, end=nend, postpone_queues=postpone_queues)
                            output += res
                            # TODO: Eher
                            if ret is not None:
                                return output, ret
                    if l == InstType.START:
                        return output, "true"
                    if l == InstType.IF or l == InstType.ELSE or l == InstType.TRY or l == InstType.CATCH:
                ...
                case Instruction(type=InstType.CALL, imm=n):
                    ...
                    pc += 1
                case Instruction(type=InstType.TRY, addr=a):
                    postpone_queues.append([])
                    active_try.append(a)
                    pc += 1
                case Instruction(type=InstType.CATCH, addr=a):
                    postpone_queues.append([])
                    pc = a
                case other:
                    assert False, other
